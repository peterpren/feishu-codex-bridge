...
export function buildPrivateCreatedCard(opts: {
  ...
  const lines = [`· 归属项目：${escapeMd(opts.projectName)}`, `· 已加入成员：${names}`, '· 这个私密群是独立 Codex 会话，默认需要 @机器人。'];
  ...
  return card(
    [
      ...
      hr(),
      md(lines.join('\n')),
      note(opts.failedParticipants?.length ? '未加入的人通常是因为机器人对该用户不可见；可让对方先私聊机器人或检查应用可用范围。' : '后续在私密群里 @机器人 即可；父群不会收到具体执行结果。'),
    ],
