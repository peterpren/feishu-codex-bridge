...
- **群 = 项目**：每个群绑定一个本地目录与默认参数；@ 机器人即在该项目边界内跑 Codex。
...
- **干活**：在项目群里 **@机器人** 描述需求；多话题群会在该群绑定目录下创建话题专属本地工作区，并在配置的飞书父文件夹下创建话题专属云文档子文件夹，流式卡片回结果。
...
- **飞书云文档隔离**：项目配置里的云文档目录只是父文件夹，不再自动授予整个群编辑权限；父文件夹只配置管理员和机器人权限。多话题群创建话题后，会自动在父文件夹下创建话题子文件夹，只给话题原始发起人编辑权限，管理员保留全量权限。若旧版本已给群授权，保存/绑定该目录时会尝试回收该群的 `openchat` 权限。
